...
# 导出版默认用于 1920x1080、Windows 缩放 100% 的电脑。
# 原始坐标是在当前电脑 125% 缩放下测量的，固定兜底点会按 100/125=0.8 换算。
...
def click_next_step(main_win, timeout=5):
    ...
    # 兜底坐标：从 125% 测量点换算到目标 100% 缩放。
    click_fixed(1037, 811, "下一步")
    return True
...
def main():
    ...
    _ACTIVE_WINDOW_ORIGIN = (win.left, win.top)
    print(f"  坐标换算: 源缩放={SOURCE_DPI_SCALE}, 目标缩放={TARGET_DPI_SCALE}, 比例={COORD_SCALE:.3f}, 窗口原点={_ACTIVE_WINDOW_ORIGIN}")
    try:
        win.activate()
    except Exception as e:
    ...
    coords = {
        "dropdown":  fixed_point(1331, 395),
        "password":  fixed_point(1331, 453),
        "checkbox":  fixed_point(1132, 489),
        "login_btn": fixed_point(1331, 577),
    }
    ...
    opt_y = coords["dropdown"][1] + round(49 * COORD_SCALE)
    ...
    if main_win is None:
        ...
        print("  [5.2] 点击 '单笔转账' ...")
        if not click_control_by_name(main_win, "单笔转账", timeout=3):
            click_fixed(162, 289, "单笔转账")
    time.sleep(2)
